...
for i_episode in trange(params["NUM_EPISODES"]):
    # sample image
    state = data_generator.sample().unsqueeze(0).to(device)
    ...
    for k in range(params["SAMPLES"]):
        ...
        eps = torch.normal(mean=torch.zeros_like(latent_mu), std=params["NOISE"]).to(device)
        ...
        reward_raw = -F.binary_cross_entropy(
            torch.tensor(next_state, requires_grad=False).permute(2, 0, 1).float(),
            state[0, :, :, :].float(), reduction='sum')
